...
import dev.langchain4j.observability.api.listener.OutputGuardrailExecutedEventListener;
...
/**
 * OpenTelemetry listener for output guardrail execution events.
 *
 ...
 */
public class OpenTelemetryOutputGuardrailListener implements OutputGuardrailExecutedEventListener {

    private static final Logger log = LoggerFactory.getLogger(OpenTelemetryOutputGuardrailListener.class);
    ...
    public void onEvent(OutputGuardrailExecutedEvent event) {
        try {
            ...
            if (parentSpan != null) {
                ...
                String eventName = "guardrail.output." + guardrailName;
            ...
            log.error("Error in OpenTelemetry output guardrail listener", e);
        }
    }
}
