...
class OpenTelemetryAiServiceListenerTest {
    ...
    @Nested
    @DisplayName("Test Case 1: Basic AiService method invocation")
    class BasicInvocationTest {
    ...
    @Nested
    @DisplayName("Test Case 2: AiService with @SystemMessage annotation")
    class SystemMessageTest {
        ...
        void shouldCaptureSystemMessageWhenContentCaptureEnabled() {
            ...
            SpanData span = spans.get(0);
            assertThat(span.getName()).isEqualTo("AiService.translate");
        ...
        void shouldNotCaptureSystemMessageWhenContentCaptureDisabled() {
            ...
            // When
            metadataOnlyListener.onEvent(startedEvent);
            metadataOnlyListener.onEvent(completedEvent);

            // Then
            List<SpanData> spans = otelExtension.getSpans();
            // Filter for the span we just created (may have spans from previous tests in same extension)
    ...
    @Nested
    @DisplayName("Test Case 3: AiService returning complex POJO")
    ...
    class ConfigurationTest {
        ...
        void shouldNotCreateSpansWhenTracingDisabled() {
            ...
            // When
            disabledListener.onEvent(startedEvent);
            disabledListener.onEvent(completedEvent);
        ...
        void shouldUseContentCaptureModeNone() {
            ...
            UUID invocationId = UUID.randomUUID();
            InvocationContext context = createInvocationContext(invocationId, "SecretService", "process");
            ...
            // When
            noCaptureListener.onEvent(startedEvent);
            noCaptureListener.onEvent(completedEvent);
    ...
    @Nested
    @DisplayName("Span naming convention tests")
    class SpanNamingTest {
